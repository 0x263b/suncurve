module SunCalc
	...
	def self.azimuth(h, phi, dec)
		Math::PI + Math::atan2(Math::sin(h), Math::cos(h) * Math::sin(phi) - Math::tan(dec) * Math::cos(phi))
	end

	def self.altitude(h, phi, dec)
		Math::PI + Math::asin(Math::sin(phi) * Math::sin(dec) + Math::cos(phi) * Math::cos(dec) * Math::cos(h))
	end
	...
	def self.hour_angle(h, phi, d)
		Math::acos((Math::sin(h) - Math::sin(phi) * Math::sin(d)) / (Math::cos(phi) * Math::cos(d))) rescue 0
	end
